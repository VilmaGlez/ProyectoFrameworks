...
public class GananciasDaoImpl implements GananciasDao {
	...
	public Integer modificarGanancia(double totalGanancias, Integer idGanancias) {
		try {
			...
			return gananciasMapper.modificarGanancia(totalGanancias, idGanancias);
		} catch (Exception e) {
			System.out.println("Error: " + e);
		}
		return null;
	...
	public double obtenerTotalGanancias() {
		List<Ganancias> list = null;
		double sumaGananciaTotal = 0.0;
		try {
			GananciasMapper gananciasMapper = sqlSession.getMapper(GananciasMapper.class);
			list = gananciasMapper.listarTodasGanancias();
			for (Ganancias c : list) {
				sumaGananciaTotal = sumaGananciaTotal + c.getTotalGanancias();
			}
			
			System.out.println("Ganancia Total = " + sumaGananciaTotal);
		} catch (Exception e) {
			System.out.println("Error: " + e);
		}
		return sumaGananciaTotal;
	}
